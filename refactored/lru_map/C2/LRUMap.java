...
public class LRUMap extends org.apache.openjpa.lib.util.collections.LRUMap
    ...
    private static final int USE_PARENT_MAX_SIZE = -1;

    private int configuredMaxSize = USE_PARENT_MAX_SIZE;
    ...
    private void validateMaxSize(int max) {
        if (max < 0) {
    ...
    @Override
    public int maxSize() {
        return usesParentMaxSize() ? super.maxSize() : configuredMaxSize;
    }

    @Override
    public boolean isFull() {
        return usesParentMaxSize()
            ? super.isFull()
            : size() >= configuredMaxSize;
    }

    private boolean usesParentMaxSize() {
        return configuredMaxSize == USE_PARENT_MAX_SIZE;
    }
