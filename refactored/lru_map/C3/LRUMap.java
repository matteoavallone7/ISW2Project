...
public class LRUMap extends org.apache.openjpa.lib.util.collections.LRUMap
    ...
    private static final int USE_INHERITED_MAX_SIZE = -1;

    private int configuredMaxSize = USE_INHERITED_MAX_SIZE;
    ...
    private static void validateMaxSize(int max) {
        if (max < 0) {
    ...
    @Override
    public int maxSize() {
        return usesInheritedMaxSize() ? super.maxSize() : configuredMaxSize;
    }

    @Override
    public boolean isFull() {
        return usesInheritedMaxSize()
            ? super.isFull()
            : size() >= configuredMaxSize;
    }

    private boolean usesInheritedMaxSize() {
        return configuredMaxSize == USE_INHERITED_MAX_SIZE;
    }
    ...
    protected void doWriteObject(ObjectOutputStream out) throws IOException {
        out.writeInt(configuredMaxSize);
